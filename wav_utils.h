...
#pragma pack(push, 1)
typedef struct riff_chunk_t {  // declare RiffChunk structure
    char chunk_id[ID_LENGTH];  // 'RIFF'
    uint32_t chunk_size;       // 4 byte size of the traditional RIFF/WAVE file
    char riff_type[ID_LENGTH]; // 'WAVE'
} riff_chunk_t, *riff_chunk_ptr;
#pragma pack(pop)

typedef struct junk_chunk_t { // declare JunkChunk structure
    char chunkId[ID_LENGTH];  // 'JUNK'
...
typedef struct format_chunk_t { // declare FormatChunk structure
    char chunk_id[ID_LENGTH];   // 'fmt '
    uint32_t chunk_size;        // 4 byte size of the 'fmt ' chunk
    /* AudioFmt_tag: Indicates the format of audio data. If the value is 1, it means using PCM format.*/
    uint16_t AudioFmt_tag;  // WAVE_FORMAT_PCM = 0x0001, etc.
...
typedef struct sub_chunk_t {
    char chunk_id[ID_LENGTH];
    uint32_t chunk_size;
    void *data;
